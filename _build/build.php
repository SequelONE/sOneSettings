...
class sOneSettingsPackage
{
    ...
    /**
     * sOneSettingsPackage constructor.
     *
     * @param $core_path
     * @param array $config
     */
    ...
    protected function settingsupd()
    {
        ...
        foreach ($settingsupd as $name => $data) {
			...
			$settingupd = $this->modx->getObject('modSystemSetting',array('key' => $name));
			if($settingupd) {
			    $settingupd->set('value',$data['value']);
		        $settingupd->save();
			} else {
...
$install = new sOneSettingsPackage(MODX_CORE_PATH, $config);
